...
| 6.5 | Tasks panel (frontend) — dockview panel rendering tasks across all projects as the task board (Kanban by status, drag-to-"ready" for agent pickup — see Task Model & Task Board); each card shows title, status badge, linked session, agent name | L      | 6.2                                        |
